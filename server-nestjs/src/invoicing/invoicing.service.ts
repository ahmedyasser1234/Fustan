...
export class InvoicingService {
  async generateInvoice(order: any): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
      ...
      doc
        .fontSize(12)
        .text(`فاتورة رقم: ${order.orderNumber}`, { align: 'right' });
      ...
      // Customer Info
      doc.text(`العميل: ${order.shippingAddress?.name || 'غير معروف'}`, {
        align: 'right',
      });
      doc.text(`العنوان: ${order.shippingAddress?.address || ''}`, {
        align: 'right',
      });
      doc.moveDown();
      ...
      doc
        .fontSize(16)
        .text(`الإجمالي الكلي: ${order.total} ر.س`, { align: 'left' });
