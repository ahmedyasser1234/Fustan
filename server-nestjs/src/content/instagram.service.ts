...
export class InstagramService {
  ...
  async syncFeed() {
    try {
      ...
      if (!businessAccountId) {
        this.logger.error(
          'Could not find Instagram Business Account ID. Ensure the account is Business/Creator and linked to a FB Page.',
        );
        return;
      }
      ...
      if (media && media.data && media.data.length > 0) {
        ...
        this.logger.log(
          `Successfully synced ${media.data.length} posts from Instagram Graph API.`,
        );
      }
    } catch (error) {
  ...
  private async getBusinessAccountId(token: string): Promise<string | null> {
    try {
      ...
      const response = await fetch(
        `${this.GRAPH_API_BASE}/me/accounts?fields=instagram_business_account&access_token=${token}`,
      );
  ...
  private async fetchStartMedia(businessId: string, token: string) {
    try {
      ...
      const response = await fetch(
        `${this.GRAPH_API_BASE}/${businessId}/media?fields=${fields}&access_token=${token}`,
      );
