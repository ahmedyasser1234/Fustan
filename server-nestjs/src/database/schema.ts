...
export const vendors = pgTable(
  'vendors',
  {
    ...
    // commissionFixed: doublePrecision('commissionFixed').default(0), // NOT in production DB
...
export const collections = pgTable(
  ...
  (table) => ({
    ...
    vendorSlugIdx: uniqueIndex('collections_vendor_slug_idx').on(
      table.vendorId,
      table.slug,
    ),
  }),
...
export const products = pgTable(
  'products',
  {
    ...
    vendorPrice: doublePrecision('vendorPrice'),
    vendorOriginalPrice: doublePrecision('vendorOriginalPrice'),
    ...
    availability: text('availability').default('sale'),
    ...
    // isCustomerListing: boolean('isCustomerListing').default(false), // NOT in production DB
...
export const offerItems = pgTable(
  ...
  (table) => ({
    ...
    uniqueOfferItem: uniqueIndex('offerItems_unique_idx').on(
      table.offerId,
      table.productId,
    ),
  }),
...
export const aiTasks = pgTable(
  'aiTasks',
  {
    id: text('id').primaryKey(), // PixVerse Task ID
    type: text('type').notNull(), // 'background_change', 'virtual_try_on'
    targetId: integer('targetId'), // Product ID or User ID
  ...
  (table) => ({
    typeIdx: index('aiTasks_type_idx').on(table.type),
    targetIdIdx: index('aiTasks_targetId_idx').on(table.targetId),
    statusIdx: index('aiTasks_status_idx').on(table.status),
  }),
);
