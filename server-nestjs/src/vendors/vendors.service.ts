...
export class VendorsService {
  ...
  async getRecentOrders(vendorId: number, limit = 5) {
    ...
    return ordersList.map(({ order, customer }) => ({
      ...
      customer,
      items: Array(
        Number(itemsCount.find((i) => i.orderId === order.id)?.count || 0),
      ),
    }));
  }
  ...
  async getCustomerDetails(vendorId: number, customerId: number) {
    ...
    const ordersList = await this.databaseService.db
      ...
      .from(orders)
      .where(
        and(eq(orders.vendorId, vendorId), eq(orders.customerId, customerId)),
      )
      .orderBy(desc(orders.createdAt));
  ...
  async updateStatus(id: number, status: string) {
    ...
    if (updated) {
      const title =
        status === 'approved' ? 'تم تفعيل حسابك' : 'تحديث حالة الحساب';
      const message =
        status === 'approved'
