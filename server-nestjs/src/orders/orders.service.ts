...
export class OrdersService {
  ...
  async create(
    ...
    const vendorGroups = new Map<
      number,
      { items: any[]; subtotal: number; vendorUserId: number }
    >();
    ...
    await this.databaseService.db.transaction(async (tx) => {
        ...
        for (const item of group.items) {
          ...
          if (productRes.length === 0)
            throw new BadRequestException(
              `Product ${item.productNameEn || item.productId} not found`,
            );

          const prod = productRes[0];

          if (item.size) {
            const sizes = prod.sizes as
              | { size: string; quantity: number }[]
              | null;
            const sizeObj = sizes?.find((s) => s.size === item.size);
            if (!sizeObj || sizeObj.quantity < item.quantity) {
              throw new BadRequestException(
                `الكمية غير متوفرة للمنتج ${prod.nameAr} مقاس ${item.size}`,
              );
            }
          } else {
            if ((prod.stock || 0) < item.quantity) {
              throw new BadRequestException(
                `الكمية غير متوفرة للمنتج ${prod.nameAr}`,
              );
            }
          }
        }
      ...
      for (const [vendorId, group] of vendorGroups) {
        ...
        const vendorOffers = await this.databaseService.db
          .select()
          .from(offers)
          .where(and(eq(offers.vendorId, vendorId), eq(offers.isActive, true)));
        ...
        for (const offer of vendorOffers) {
          ...
          if (offerProductIds.length === 0) {
            ...
            // Specific products
            matchingItems = group.items.filter((item) =>
              offerProductIds.includes(item.productId),
            );
          }
        ...
        const [newOrder] = await tx
          ...
          .values({
            ...
            stripePaymentId:
              paymentMethod === 'card' ? `mock_tx_${Date.now()}` : null,
        ...
        if (newOrder.paymentStatus === 'paid') {
          await this.walletsService.handleOrderPaid(
            vendorId,
            newOrder.id,
            vendorEarnings,
          );
  ...
  async updateStatus(orderId: number, status: string, userId?: number) {
    ...
    if (newStep < currentStep && newStatusNormalized !== 'cancelled') {
      throw new BadRequestException(
        'Cannot revert order to a previous status.',
      );
    }
      ...
      if (user) {
        ...
        if (user.role === 'vendor') {
          ...
          if (newStatusNormalized === 'delivered') {
            throw new BadRequestException(
              'Vendors cannot mark orders as Delivered. Only customers can confirm receipt.',
            );
          }
        }
      }
    ...
    if (updatedOrder) {
      ...
      if (newStatusNormalized === 'delivered' && vendorUserId) {
        ...
        const vendorEarnings =
          ...
          Number(updatedOrder.commission) -
          Number(updatedOrder.shippingCost || 0);
        const vendorId = updatedOrder.vendorId;
        if (vendorId) {
          await this.walletsService.handleOrderDelivered(
            vendorId,
            updatedOrder.id,
            vendorEarnings,
          );
