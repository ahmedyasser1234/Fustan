...
export class AdminService {
  ...
  async onModuleInit() {
    // One-time cleanup for the developer backdoor admin (ID=1) as per security report
    try {
      await this.databaseService.db
        .delete(users)
        .where(and(eq(users.id, 1), eq(users.email, 'dev@fustan.com')));
    } catch (error) {
      // Might already be deleted or ID 1 is different, safe to ignore
    }
  }
  ...
  async getAllProducts(search?: string) {
    let query = this.databaseService.db.select().from(products);
    ...
    return await this.databaseService.db.transaction(async (tx) => {
      ...
      if (productIds.length > 0) {
        ...
        await tx
          ...
          .where(sql`${productColors.productId} IN ${productIds}`);
        ...
        await tx
          .delete(cartItems)
          .where(sql`${cartItems.productId} IN ${productIds}`);
        ...
        await tx
          .delete(shipping)
          .where(sql`${shipping.productId} IN ${productIds}`);

        // 6. Delete Offer Items
        await tx
          .delete(offerItems)
          .where(sql`${offerItems.productId} IN ${productIds}`);
      ...
      if (offerIds.length > 0) {
        await tx
          .delete(offerItems)
          .where(sql`${offerItems.offerId} IN ${offerIds}`);
  ...
  async deleteCustomer(id: number, adminEmail: string) {
    ...
    if (adminEmail !== MAIN_ADMIN_EMAIL) {
      throw new UnauthorizedException(
        'Only the main admin can delete customers.',
      );
    }
  ...
  async globalSearch(query: string) {
    ...
    const [vendorsResults, productsResults, customersResults, ordersResults] =
      await Promise.all([
